...
const normalizeData = (unnormalizedData: { id: string, title: string, body: string }[] ): object => {
    let ObjbyId = {}
    unnormalizedData.map((arr) => ObjbyId = {...ObjbyId, [arr.id]: arr} )
    return {
        byId: ObjbyId,
        allIds: [unnormalizedData.map(arr => arr.id)],
    }
};
